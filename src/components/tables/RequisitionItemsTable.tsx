              ...
              <tr className='border border-black'>
                {columns.map((columnName) => (
                  columnName === 'Quantidade' ?
                     <th className="px-4 w-1/4 py-3 border text-black"><h1 className='text-center'>{columnName}</h1></th>
                    : <th className="px-4  py-3 border text-black"><h1 className='text-center'>{columnName}</h1></th>
                ))}
              </tr>
            </thead>
            <tbody className=''>
              ...
              <>
                  <tr className={ item.ATIVO && item.ATIVO > 0 ? ` border` : ` border bg-gray-300`}>
                            <td
                              scope="row"
                              className={item.ATIVO && item.ATIVO > 0 ?
                                ...
                                `text-sm px-4 py-2 text-gray-600 whitespace-nowrap border` }>
                                    ...
                                    >{item.OBSERVACAO !== 'null' ? item.OBSERVACAO  : 'observação'}
                                      </button>
                            ...
                            <td
                              ...
                              className={item.ATIVO && item.ATIVO > 0 ?
                                `text-sm px-4 py-2 text-gray-900 whitespace-nowrap border`
                                  : `text-sm px-4 py-2 text-gray-600 whitespace-nowrap border`}>
                              <p className='overflow-x-auto  max-w-[500px]'>{item.codigo}</p>
                            </td>
                            <td
                              scope="row"
                              className={item.ATIVO && item.ATIVO > 0 ?
                            ...
                            <td className="px-4 py-2 flex gap-1 items-center justify-center">
                              ...
                              >
                                <EditIcon className={item.ATIVO && item.ATIVO > 0 ? `cursor-pointer text-blue-600` : `cursor-pointer text-blue-gray-100`} />
                              </button>
                              <button
                                id={String(item.ID_PRODUTO)}
                                className={item.ATIVO && item.ATIVO > 0 ? `cursor-pointer text-red-600` : `cursor-pointer text-blue-gray-100`}
                                onClick={() => {
                              ...
                              <Switch
                                ...
                                onChange={() => item.ATIVO && item.ATIVO > 0 ? handleCancelItem(item) : handleActivateItem(item)} />
                            </td>       
                </tr>
              </>
                  ))
